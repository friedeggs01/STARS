...
def get_actual_output(test_code: str):
    ...
    # print("Modified code for execution:\n", modified_code)
    ...
    # print(f"Actual output: {actual_output}, Expected output: {expected}")
...
def check_correctness(file_path, generated_data,ks=[1, 2, 5, 10, 15, 20]):
    ...
    for i, data in tqdm(enumerate(generated_data)):
        ...
        with open(f'tmp_{i}_{difficulty}/under_test.py','w') as f: #write program under test and test cases into tmp files
            f.write(code)
        passed_tests=[]
        # print(f'Wrote under_test.py for task {i} to tmp_{i}_{difficulty}/under_test.py')
        
        for j, testcase in enumerate(test_cases):
            #testcase=textwrap.dedent(testcase)
            total_cases+=1
            print("Total syntax correct:", total_syn_correct/total_cases)
            # print("Total executable correct:", total_exec_correct/total_cases)
            print("Total assertion correct:", total_assert_correct/total_cases)
            try:
                ...
                if res==True:
                    if test_code.find(f'solution.{func_name}')==-1: #if the function under test is not called, also consider as failed
                        ...
                        # print("Total executable correct:", total_exec_correct)
                        ...
                        # --- BEGIN assertion correctness ---
                        # print("Checking assertion correctness...")
                        # print("Test code:\n", test_code)
                        is_correct, actual_output, expected_output = get_actual_output(test_code)
                        # print("Assertion correctness:", is_correct)
                        if is_correct: 
                            total_assert_correct += 1
                            print(f'Test {j} - Actual: {actual_output}, Expected: {expected_output}, Correct: {is_correct}')
                        else: 
                            print(f'Test {j} - Actual: {actual_output}, Expected: {expected_output}, Correct: {is_correct}')
                        # --- END assertion correctness ---
        ...
        if len(passed_tests)>0: #start measuring coverage
            ...
            # result = subprocess.run(cov_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            # print(result.stdout.decode())
            # print(result.stderr.decode())
            # print("coverage.json exists:", os.path.exists('coverage.json'))
    ...
    assert_correct=total_assert_correct/total_cases
    ...
    row = {
        ...
        "Assertion Correctness": assert_correct
    }
    ...
    for k in ks:
        ...
        row[f'line_cov@{k}'] = line_covs_at_k[f'cov@{k}']
        row[f'branch_cov@{k}'] = branch_covs_at_k[f'cov@{k}']
        
    #compute coverage
    avg_line_cov=total_line_cov/len(generated_data)
    avg_branch_cov=total_branch_cov/len(generated_data)
    row[f'avg_line_cov'] = avg_line_cov
    row[f'avg_branch_cov'] = avg_branch_cov
    print(f'Average Line Coverage: {avg_line_cov}, Average Branch Coverage: {avg_branch_cov}')

    # Đường dẫn file CSV
    csv_path = "metrics.csv"

    # Ghi CSV (append hoặc tạo mới)
    try:
        with open(csv_path, "x", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=row.keys())
            writer.writeheader()  # Chỉ ghi header khi file mới tạo
            writer.writerow(row)
    except FileExistsError:
        with open(csv_path, "a", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=row.keys())
            writer.writerow(row)  # Chỉ append row, không ghi header nữa
...
if __name__=='__main__':
    ...
    print(args.path)
    print(args.ks)
    output_dir = Path('predictions_svd')
    predictions=read_jsonl(args.path)
    print(len(predictions))

    check_correctness(args.path, predictions, ks=args.ks)
